...
export function withActiveSpanSync(tracerNameOrTracer, spanName, spanAttributes, callback) {
    ...
    try {
        const result = api.context.with(span, () => callback(span))
        return result
    } catch (e) {
        handleException(span, e)
        throw e
...
export function createTracer(name, { version, filepath } = {}) {
    ...
    function prepareAttributes(spanAttributes, callback) {
    ...
    return {
        ...
        withActiveSpan: (spanName, spanAttributes, callback) => {
            const { attributes, callback: actualCallback } = prepareAttributes(spanAttributes, callback)
            return withActiveSpan(tracer, spanName, attributes, actualCallback)
        },
        /**
         * 
         * @param {String} spanName the name of the span
        ...
        withActiveSpanSync: (spanName, spanAttributes, callback) => {
            const { attributes, callback: actualCallback } = prepareAttributes(spanAttributes, callback)
            return withActiveSpanSync(tracer, spanName, attributes, actualCallback)
        },
        /**
         * 
         * @param {String} spanName the name of the span
        ...
        withOrpanedSpan: (spanName, spanAttributes, callback) => {
            const { attributes, callback: actualCallback } = prepareAttributes(spanAttributes, callback)
            return withOrpanedSpan(tracer, spanName, attributes, actualCallback)
        }
    }
}   